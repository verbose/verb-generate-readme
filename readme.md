...
[Silent](#Silent) task that loads data to used for rendering templates. Called by the [readme](#readme) task.
...
### [verbmd](generator.js#L143)

[Silent](#Silent) task that loads the `.verb.md` in the user's current working directory. If no `.verb.md` file exists, the [prompt-verbmd)() task is called to ask the user if they want to add the file. Disable the prompt by passing `--verbmd=false` on the command line, or `app.disable('verbmd')` via API.
...
### [prompt-verbmd](generator.js#L177)
